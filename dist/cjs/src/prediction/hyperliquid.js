...
var hyperliquid$1 = require('../abstract/prediction/hyperliquid.js');
...
class hyperliquid extends hyperliquid$1["default"] {
        ...
        return this.deepExtend(super.describe(), {
            ...
            'exceptions': {
                'exact': {
                    ...
                    'Insufficient spot balance': errors.InsufficientFunds,
                    'Too many cumulative requests sent': errors.RateLimitExceeded,
    ...
    async fetchPositions(outcomes = undefined, params = {}) {
        ...
        for (let i = 0; i < balances.length; i++) {
            ...
            if ((totalStr === undefined) || Precise["default"].stringEq(totalStr, '0')) {
                continue;
            }
    ...
    parsePredictionPosition(position, market = undefined) {
        ...
        if ((entryNtlStr !== undefined) && (totalStr !== undefined) && !Precise["default"].stringEq(totalStr, '0')) {
            entryPrice = this.parseNumber(Precise["default"].stringDiv(entryNtlStr, totalStr));
        }
        ...
        if ((markPxStr !== undefined) && (totalStr !== undefined)) {
            const notionalStr = Precise["default"].stringMul(totalStr, markPxStr);
            notional = this.parseNumber(notionalStr);
            if (entryNtlStr !== undefined) {
                unrealizedPnl = this.parseNumber(Precise["default"].stringSub(notionalStr, entryNtlStr));
            }
        }
    ...
    resolveOutcomeInput(outcomeInput) {
        if (outcomeInput === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' resolveOutcomeInput() requires an outcome symbol or id');
        }
        if (this.outcomes === undefined || this.outcomes_by_id === undefined) {
            throw new errors.ExchangeError(this.id + ' outcomes not loaded');
        }
        ...
        throw new errors.ArgumentsRequired(this.id + ' cannot resolve outcome from input: ' + outcomeInput + '. Provide an outcome symbol (e.g. MARKET:YES), outcome id (#<encoding>), or market id with side.');
    }
    /**
     * @method
    ...
    async createOrder(outcome, type, side, amount, price = undefined, params = {}) {
        ...
        if (price === undefined) {
            if (isMarket) {
                throw new errors.ArgumentsRequired(this.id + ' createOrder() requires a reference price for market orders on outcome markets in between 0 and 1. The exchange uses this reference price together with the configured slippage to derive the execution price.');
            }
    ...
    async cancelOrders(ids, outcome = undefined, params = {}) {
        ...
        if (outcome === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' cancelOrders() requires an outcome argument');
        }
        await this.initializeClient();
        await this.loadOutcome(outcome);
        const outcomeObj = this.outcome(outcome);
        ...
        for (let i = 0; i < statuses.length; i++) {
            ...
            if (error !== undefined) {
                throw new errors.OrderNotFound(this.id + ' cancelOrders() failed for ' + this.safeString(requestIds, i, this.safeString(requestIds, 0)) + ': ' + error);
            }
            const success = (status === 'success') || (this.safeString(status, 'status') === 'success');
            if (!success) {
                throw new errors.ExchangeError(this.id + ' cancelOrders() received an unexpected status: ' + this.json(status));
            }
    ...
    async fetchOrder(id, outcome = undefined, params = {}) {
        ...
        if (outcome !== undefined) {
            ...
            if (this.safeString(parsed, 'outcome') !== expected) {
                throw new errors.OrderNotFound(this.id + ' fetchOrder() order ' + id + ' is not in outcome ' + expected);
            }
        }
        return parsed;
    }
