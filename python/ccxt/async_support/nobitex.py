...
class nobitex(Exchange):
    ...
    async async def fetch_tickers(self, symbols=None, params={}):
        await self.load_markets()
        result = {}
    ...
    async async def fetch_order_book(self, symbol, limit=None, params={}):
        await self.load_markets()
        market = self.market(symbol)
        request_symbol = market['baseId'].upper() + market['quoteId'].upper()
        response = await self.request('v3/orderbook/' + request_symbol, 'public', 'GET', params)
        timestamp = self.milliseconds()
    ...
    async def fetch_trades(self, symbol, since=None, limit=None, params={}):
        ...
        market = self.market(symbol)
        request_symbol = market['baseId'].upper() + market['quoteId'].upper()
        response = await self.request('v2/trades/' + request_symbol, 'public', 'GET', params)
    ...
    async async def fetch_orders(self, symbol=None, since=None, limit=None, params={}):
        await self.load_markets()
        market = None
        request = {}
        if symbol:
    ...
    async def fetch_profile(self, params={}):
        return self.request('users/profile', 'private', 'GET', params)

    async def fetch_transactions_history(self, params={}):
        response = await self.request('users/transactions-history', 'private', 'GET', params)
        return self.safe_value(response, 'transactions', [])

    async def fetch_favorite_markets(self, params={}):
